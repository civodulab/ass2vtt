    ...
    fs.readdir(chemin, function (err, items) {
        items.forEach(f => {
            if (path.extname(f) === '.ssa') {
                monLog.error('Votre fichier ' + f + ' est au format ssa. Le convertir en ass pour y avoir la taille de la vidéo.');
                return;
            }
...
function writeFiles(options, file_in) {
    file_in.forEach(f => {
        ...
        const argv = {
            'file_in': f,
            'file_out_classic': f.split('.')[0] + '_classic.vtt',
            'file_out_a11y': f.split('.')[0] + '_a11y.vtt',
        };
        if (options.both || (!options.classic && !options.a11y)) {
            // classique
            fs.writeFileSync(argv.file_out_classic, mesFunctions.writeFile(parse).file_classic);
            monLog.log('ass2vtt', argv.file_out_classic, 'généré');
            // accessible
            fs.writeFileSync(argv.file_out_a11y, mesFunctions.writeFile(parse).file_a11y);
